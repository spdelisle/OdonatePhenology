...
by name2 lat year rownum_char;
run;

/*
proc glimmix data = lswed order = data;
class  name rownum_char _NAME_;
model COL1 = _NAME_/ noint;
random _NAME_ / sub = name type = lin(3) ldata = bv;
random _NAME_ / sub = rownum_char type = un rside;
*/
...
model COL1 = _NAME_/ noint;
random _NAME_ / sub = name type = un;
random _NAME_ / sub = lat(name) group = name type = un;
random _NAME_/ sub = year(lat) group = name type = un;
repeated _NAME_/ subject = rownum_char type = un;
*/
/*
proc hpmixed data = sweden  order = data;
class name lat year;
model day = one20;
random intercept one20/ sub = name type = un;
random intercept one20/ sub = lat(name) type = un;
random intercept one20/ sub = year(lat*name) type = un;
*/

/*
proc glimmix method = laplace data = sweden  order = data;
class name ;
model day = one20;
random intercept one20/ sub = name type = lin(3) ldata = bv;
*/
/*
proc glimmix data = sweden  initglm order = data;
class name lat year;
model day = one20/ s;
random intercept one20/ sub = name type = lin(3) ldata = bv solution;
random intercept one20 / sub = lat(name) type = un ;
proc glimmix data = sweden  initglm order = data;
class name  year;
model day = one20/ s;
random intercept one20/ sub = name type = lin(3) ldata = bv solution;
random intercept one20 / sub = year(name) type = un ;
*/


/*
proc standard data = lswed out = stds mean = 0;
var COL1;
by name _NAME_;
*/
/*
...
ods output ParameterEstimates = means(keep = Estimate name);
proc hpmixed data = lswed;
class  name rownum_char _NAME_ lat year;
model COL1 = _NAME_/s noint;
...
proc hpmixed data = sweden;
class  name rownum_char lat year;
model day = y y2 /s noint;
random _intercept_ / sub = lat type = vc;
random _intercept_ / sub = year type = vc;
proc hpmixed data = sweden;
class  name rownum_char lat year;
model one20 = y y2 /s noint;
random _intercept_ / sub = lat type = vc;
random _intercept_ / sub = year type = vc;

proc hpmixed data = stds;
class  name rownum_char _NAME_ lat year;
model COL1 = _NAME_ /s noint;
random _NAME_ / sub = lat type = vc;
random _NAME_ / sub = year type = vc;
by name;
*/
...
by name;
ods listing;
proc print data = fitFull;
proc print data = fitRed;
*/
RUN;
